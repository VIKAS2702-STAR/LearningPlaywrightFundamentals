...
- Name: 06_Multiple_Elements\265_Multi_Element_Direct.spec.ts >> Basic Verify how to handle multiple elements
...
Call log:
  - waiting for getByTestId('forgotten - password - link')
...
```ts
  ...
  2  | 
  3  | test( 'Basic Verify how to handle multiple elements', async ( { page } ) => {
  4  | 
  5  |     // Navigate to the page.
  6  |     // Find the locator which gives all the elements and text
  7  |     // loop through it and find the one which we want to click
  8  | 
  9  |     await page.goto( "https://app.thetestingacademy.com/playwright/multiple_element_filter" );
  10 | 
> 11 |     await page.getByTestId( 'forgotten - password - link' ).click();
     |                                                             ^ Error: locator.click: Test timeout of 30000ms exceeded.
  12 |     await page.pause();
  13 | 
  14 | } );
